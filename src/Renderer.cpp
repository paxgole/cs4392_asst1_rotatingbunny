...
void Renderer::nanogui_init(GLFWwindow* window) {
	...
	gui_1->addGroup("Model Position");
	gui_1->addVariable("X", m_animation->angleX)->setSpinnable(true);
	gui_1->addVariable("Y", m_animation->angleY)->setSpinnable(true);
	// gui_1->addVariable("Z", m_animation->position[2])->setSpinnable(true);
	//////////
	...
	gui_1->addButton("Local Rotate X", []()
	{
		m_animation->updateX();
	});

	gui_1->addButton("Global Rotate Y", []()
	{
		m_animation->updateY();
	});
